  ...
  return <Card><CardHeader><h2 className="font-semibold text-white">Loader state</h2></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">
    {orderedLinks.map((link) => {
      ...
      return <div key={link.linkIndex} className="rounded-lg border border-bambu-dark-tertiary bg-bambu-dark/40 p-4">
        ...
        <div className={`mt-4 grid grid-cols-3 gap-2 text-xs ${live ? '' : 'opacity-50'}`}><div><span className="block text-bambu-gray">Motion</span><span className="text-white">{link.motion ?? '—'}</span></div><div><span className="block text-bambu-gray">Pull</span><span className="text-white">{link.pullPercent == null ? '—' : `${link.pullPercent}%`}</span></div><div><span className="block text-bambu-gray">Pressure</span><span className="text-white">{link.pressure ?? '—'}</span></div></div>
        {!live && <div className="mt-3 flex items-center gap-1.5 rounded bg-amber-500/10 px-2 py-1.5 text-xs text-amber-300"><Clock className="h-3.5 w-3.5 flex-shrink-0" />{age == null ? 'No loader state has been received for this link.' : `Loader state is ${age} old — the bridge has sent no STATUS since.`}</div>}
        <div className="mt-3 flex items-center justify-between text-xs text-bambu-gray"><span className="flex items-center gap-1"><CircleGauge className="h-3.5 w-3.5" />filament 0x{link.activeMask.toString(16).padStart(2, '0')}</span><span className={link.faultCount ? 'flex items-center gap-1 text-red-400' : 'flex items-center gap-1'}><AlertTriangle className="h-3.5 w-3.5" />{link.faultCount} faults</span></div>
      </div>;
    })}
  </CardContent></Card>;
}
