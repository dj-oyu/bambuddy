...
export function HardwareMetrics({ points }: { points: BMCUMetricPoint[] }) {
  const chronological = [...points].sort((a, b) => bmcuTimestamp(a.at) - bmcuTimestamp(b.at));
  const latest = chronological.at(-1);
  const data = chronological.map((p) => ({ ...p, time: bmcuTimestamp(p.at), heapFreeKb: p.heapFreeBytes == null ? null : Math.round(p.heapFreeBytes / 1024) }));
  const summaries = [
  ...
  return <Card><CardHeader><h2 className="font-semibold text-white">Pico hardware</h2><p className="text-xs text-bambu-gray">Window values are charted; cumulative counters are shown as totals or rates.</p></CardHeader><CardContent>
    ...
    {data.length > 1 && <div className="mt-5 h-44" aria-label="Pico hardware metrics chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={data}><CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis dataKey="time" type="number" domain={['dataMin', 'dataMax']} tickFormatter={(v) => formatBMCUTime(Number(v))} stroke="#94a3b8" /><YAxis stroke="#94a3b8" /><Tooltip labelFormatter={(v) => formatBMCUDateTime(Number(v))} /><Line dataKey="heapFreeKb" name="Free heap (KB)" stroke="#22c55e" dot={false} /><Line dataKey="loopGapP99Us" name="Loop p99 (µs)" stroke="#f59e0b" dot={false} /><Line dataKey="transportSendMaxUs" name="Send max (µs)" stroke="#ef4444" dot={false} /></LineChart></ResponsiveContainer></div>}
