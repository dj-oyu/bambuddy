...
export function LoaderTimeline({ points }: { points: BMCUTimelinePoint[] }) {
  ...
  const data = points.map((point) => ({ ...point, time: new Date(point.at).getTime(), eventY: point.anomaly || !continuousKinds.has(point.kind) ? (point.pullPercent ?? 50) : null }));
  return <Card>
    ...
    <CardContent>
      {data.length === 0 ? <div className="py-14 text-center text-bambu-gray">No timeline data in this range.</div> : <div className="h-72" aria-label="BMCU loader event and anomaly graph">
        <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 10, right: 12, bottom: 4, left: -12 }}>
          ...
          <XAxis dataKey="time" type="number" domain={['dataMin', 'dataMax']} tickFormatter={(v) => new Date(v).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} stroke="#94a3b8" />
          <YAxis domain={[0, 100]} stroke="#94a3b8" unit="%" />
          <Tooltip labelFormatter={(v) => new Date(Number(v)).toLocaleString()} />
          {points.filter((p) => p.missingData).map((p) => { const at = new Date(p.at).getTime(); return <ReferenceArea key={p.id} x1={at - 15000} x2={at + 15000} fill="#64748b" fillOpacity={0.18} />; })}
