export default {
  ...
  printers: {
    ...
    filamentSpoof: {
      badgeTitle: '{{slot}} が使い切られると自動的に引き継ぎます',
      menuEngage: '使い切りバックアップを設定',
      menuRelease: 'バックアップを解除',
      modalTitle: '使い切りバックアップ',
      primaryLabel: '{{slot}} のバックアップ',
      modalHelp: 'このスロットのフィラメントが切れたときに自動的に引き継ぐスプールを選択してください。',
      sameTypeOnly: '同じ材料タイプ({{type}})のスプールのみ引き継げます。',
      noCandidates: '互換性のあるスプールがありません — バックアップは同じ材料タイプ({{type}})である必要があります。',
      confirm: 'バックアップを設定',
      toastEngaged: '使い切りバックアップを設定しました',
      toastEngagedNative: 'バックアップを有効にしました — スプールはすでに一致しています。',
      toastReleased: 'バックアップを解除しました',
      toastFailed: '使い切りバックアップを有効にできませんでした — プリンターが変更を受け付けませんでした。もう一度試すか、プリンターを再起動してください。',
      kDiffWarning: 'このスプールのフロー特性がわずかに異なります。切り替え後に印刷品質が変わる場合があります。',
      badgePending: '{{slot}} の使い切りバックアップを有効化しています…',
      reassignTitle: 'バックアップを再割り当てしますか？',
      reassignBody: 'このスプールは現在、同じ色の別のスプールのバックアップになっています。再割り当てしますか？',
      reassignConfirm: '再割り当て',
    },
    activeJobSlot: {
  ...
  settings: {
    ...
    bmcuLink: {
      infoTitle: 'BMCU Link デバイス',
      infoBody: 'BMCU Link アダプタは接続時に自動で登録されます。このパネルではアダプタごとのリンク状態、最新のステータススナップショット、最近のテレメトリイベントを確認できます。',
      disabled: 'このサーバーでは BMCU Link が無効です。',
    ...
    gcodeTemplateInsert: 'テンプレートを挿入',
    gcodeTemplateOverwriteConfirm: 'このフィールドの既存のG-codeが上書きされます。続行しますか？',
  ...
  gcodeMotion: {
    title: 'G-codeモーションプレビュー',
    previewButton: '動きをプレビュー',
    play: '再生',
    pause: '一時停止',
    scrubber: 'タイムライン',
    speed: '速度',
    frontOverhang: '手前への突き出し',
    backOverhang: '奥への突き出し',
    corexyNoSwing: 'CoreXY: ベッドは前後に動きません（Zのみ）',
    unknownGeometry: '形状不明 — 汎用256mmプリンターで表示しています',
    accelDisclaimer: '所要時間は加減速を無視した目安です。実機の動きはこれより遅くなります。',
  },
  libraryTrash: {
    title: 'ゴミ箱',
    headerButton: 'ゴミ箱',
