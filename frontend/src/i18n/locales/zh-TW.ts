export default {
  ...
  printers: {
    ...
    filamentSpoof: {
      badgeTitle: '當 {{slot}} 用盡時自動接替',
      menuEngage: '設定斷料備用',
      menuRelease: '移除斷料備用',
      modalTitle: '斷料備用',
      primaryLabel: '{{slot}} 的備用',
      modalHelp: '選擇當此槽位耗盡時自動接替的線材捲。',
      sameTypeOnly: '只有相同材料類型({{type}})的線材捲可以接替。',
      noCandidates: '沒有相容的線材捲 — 備用必須是相同材料類型({{type}})。',
      confirm: '設定備用',
      toastEngaged: '已設定斷料備用',
      toastEngagedNative: '已啟用備用 — 線材捲已相符。',
      toastReleased: '已移除斷料備用',
      toastFailed: '無法啟用斷料備用 — 印表機未接受此變更。請重試或重新啟動印表機。',
      kDiffWarning: '此線材捲的流動特性略有不同；切換後列印品質可能會有所變化。',
      badgePending: '正在為 {{slot}} 啟用斷料備用…',
      reassignTitle: '重新指派備用？',
      reassignBody: '此線材捲目前是另一捲相同顏色線材捲的備用。要重新指派嗎？',
      reassignConfirm: '重新指派',
    },
    activeJobSlot: {
  ...
  settings: {
    ...
    gcodeTemplateInsert: '插入範本',
    gcodeTemplateOverwriteConfirm: '將覆蓋此欄位中現有的 G-code。是否繼續？',
  ...
  gcodeMotion: {
    ...
    frontOverhang: '前方伸出',
    backOverhang: '後方伸出',
    corexyNoSwing: 'CoreXY：熱床不會前後移動（僅 Z 軸）',
    unknownGeometry: '未知機型 — 使用通用 256mm 印表機顯示',
    accelDisclaimer: '時長未考慮加減速，實際運動會更慢。',
  },
  libraryTrash: {
    title: '資源回收筒',
    headerButton: '資源回收筒',
