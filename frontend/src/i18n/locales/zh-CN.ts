export default {
  ...
  printers: {
    ...
    filamentSpoof: {
      badgeTitle: '当 {{slot}} 用尽时自动接替',
      menuEngage: '设置断料备用',
      menuRelease: '移除断料备用',
      modalTitle: '断料备用',
      primaryLabel: '{{slot}} 的备用',
      modalHelp: '选择当此槽位耗尽时自动接替的料卷。',
      sameTypeOnly: '只有相同材料类型({{type}})的料卷可以接替。',
      noCandidates: '没有兼容的料卷 — 备用必须是相同材料类型({{type}})。',
      confirm: '设置备用',
      toastEngaged: '已设置断料备用',
      toastEngagedNative: '已启用备用 — 料卷已匹配。',
      toastReleased: '已移除断料备用',
      toastFailed: '无法启用断料备用 — 打印机未接受此更改。请重试或重启打印机。',
      kDiffWarning: '此料卷的流动特性略有不同；切换后打印质量可能会有所变化。',
      badgePending: '正在为 {{slot}} 启用断料备用…',
      reassignTitle: '重新分配备用？',
      reassignBody: '此料卷目前是另一卷相同颜色料卷的备用。要重新分配吗？',
      reassignConfirm: '重新分配',
    },
    activeJobSlot: {
  ...
  settings: {
    ...
    gcodeTemplateInsert: '插入模板',
    gcodeTemplateOverwriteConfirm: '将覆盖此字段中现有的 G-code。是否继续？',
  ...
  gcodeMotion: {
    ...
    frontOverhang: '前方伸出',
    backOverhang: '后方伸出',
    corexyNoSwing: 'CoreXY：热床不会前后移动（仅 Z 轴）',
    unknownGeometry: '未知机型 — 使用通用 256mm 打印机显示',
    accelDisclaimer: '时长未考虑加减速，实际运动会更慢。',
  },
  libraryTrash: {
    title: '回收站',
    headerButton: '回收站',
