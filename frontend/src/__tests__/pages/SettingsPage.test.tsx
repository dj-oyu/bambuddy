...
describe('SettingsPage', () => {
  ...
  describe('general settings', () => {
    ...
    it('saves mixed Sidebar order when items are dragged', async () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith(
        SIDEBAR_ORDER_KEY,
        JSON.stringify(['ext-7', 'printers', 'inventory', 'archives', 'queue', 'projects', 'files', 'makerworld', 'profiles', 'maintenance', 'stats', 'bmcu-link', 'finance', 'notifications', 'settings']),
      );
    });
    ...
    it('resets Sidebar to all pages first and configured links at the bottom', async () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith(
        SIDEBAR_ORDER_KEY,
        JSON.stringify(['printers', 'inventory', 'archives', 'queue', 'projects', 'files', 'makerworld', 'profiles', 'maintenance', 'stats', 'bmcu-link', 'finance', 'notifications', 'settings', 'ext-7']),
      );
  ...
  describe('external camera snapshot URL override (#1177)', () => {
    ...
    it(
      ...
      // mode on PR #1263). Private fork: raised further to 60s — the same
      // failure mode reproduces on the RDK X5 (ARM) dev board.
      60_000,
    );
  });
