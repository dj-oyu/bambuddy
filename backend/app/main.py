...
def _format_hms_error_summary(hms_errors: list[dict]) -> str | None:
    ...
    if not hms_errors:
        return None
    from backend.app.services.hms_errors import get_error_description, get_error_description_full

    parts: list[str] = []
    for err in hms_errors:
        try:
            attr_int = int(err.get("attr", 0))
            code_value = err.get("code", 0)
            code_text = str(code_value).replace("0x", "")
            code_int = int(code_text, 16) if code_text else 0
            ...
            short_code = _hms_short_code(attr_int, code_value)
        except (TypeError, ValueError):
            continue
        description = (
            err.get("description")
            ...
            try:
                ...
                async with async_session() as db:
                    ...
                    for error in new_errors:
                        ...
                        description = getattr(error, "description", None) or get_error_description_full(
                            error.attr, error_code_int
                        )
                        if (
                            not description
...
_FEED_STALL_LINK_ID = os.environ.get("BAMBUDDY_FEED_STALL_LINK_ID", "").strip()
    ...
    async with async_session() as db:
        ...
        if is_pause_stage:
            ...
            if paused:
                ...
                lines.append("PAUSE was sent, but the printer did not confirm the PAUSE state.")
            else:
...
async def _pause_feed_stall_with_confirmation(printer_id: int) -> bool:
    """Publish PAUSE and only succeed after MQTT state confirms it."""
    log = logging.getLogger(__name__)
    client = printer_manager.get_client(printer_id)
    if client is None:
        return False
    for attempt in range(1, 4):
        if not client.pause_print():
            log.error("[FEED-STALL] PAUSE publish attempt %s failed for printer %s", attempt, printer_id)
        for _ in range(6):
            await asyncio.sleep(0.5)
            status = printer_manager.get_status(printer_id)
            state = (getattr(status, "state", "") or "").upper() if status is not None else ""
            if state == "PAUSE":
                log.warning(
                    "[FEED-STALL] printer %s confirmed PAUSE after publish attempt %s",
                    printer_id,
                    attempt,
                )
                return True
            if state and state != "RUNNING":
                log.warning(
                    "[FEED-STALL] printer %s left RUNNING without PAUSE confirmation (state=%s)",
                    printer_id,
                    state,
                )
                return False
    final_status = printer_manager.get_status(printer_id)
    final_state = (getattr(final_status, "state", "") or "unknown").upper()
    log.error(
        "[FEED-STALL] PAUSE unconfirmed for printer %s after 3 publishes (state=%s)",
        printer_id,
        final_state,
    )
    return False
...
async def _feed_stall_tick(now: float) -> None:
    ...
    if isinstance(event, FeedStallTrigger):
        ...
        if _FEED_STALL_AUTO_PAUSE:
            paused = await _pause_feed_stall_with_confirmation(printer_id)
            if not paused:
