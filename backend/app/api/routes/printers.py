...
async def get_printer_status(
    ...
    # Filament-spoof status fields (shared contract with the WS serializer).
    from backend.app.services.filament_spoof import pending_spoof_map, spoof_status_fields

    _spoof_pending = pending_spoof_map(printer_manager.get_client(printer_id))
        ...
        for ams_data in raw_data["ams"]:
            ...
            for tray_data in ams_data.get("tray", []):
                ...
                _spoof_state, _spoof_primary = spoof_status_fields(
                    tray_data, ams_data.get("id", 0), tray_data.get("id", 0), _spoof_pending
                )
