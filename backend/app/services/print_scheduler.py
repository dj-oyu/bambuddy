...
class PrintScheduler:
    ...
    async def _start_print(self, db: AsyncSession, item: PrintQueueItem):
        ...
        if item.archive_id:
            ...
            if not library_file:
                ...
                await self._fail_queue_item(db, item, error_message, reason="library file missing")
                logger.error(
    ...
    async def _watchdog_print_start(
        ...
        async def _do_revert(db):
            ...
            # #2732: a command-verification refusal has no retry budget — the
            # printer refused to verify the command, and re-uploading the same
            # 3MF to the same printer will be refused the same way. Fail now
            # with the fix rather than after three laps of a message about SD
            # cards.
            gave_up = command_rejected or attempts >= DISPATCH_MAX_ATTEMPTS
            extra = {
            ...
            if command_rejected:
                extra.update(
                    {
                        "error_message": (
                            "The printer rejected the print command: MQTT command verification "
                ...
                failure_message = (
                    (
                        ...
                        f"{attempts} attempts. {', '.join(f'AMS {i}' for i in drying_ams_ids)} "
                        f"{'was' if len(drying_ams_ids) == 1 else 'were'} drying throughout — "
                        "some printers refuse to begin a print while an AMS is in a drying "
                        "cycle, and an AMS drying without its external power supply can also "
                        "leave too little power for the start-of-print calibration. Stop the "
                        "drying, or connect the AMS power supply, and start the job again."
                    )
                    if drying_ams_ids
                    else (
