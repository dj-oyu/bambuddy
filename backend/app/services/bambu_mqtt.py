...
# Private fork (#1867): fire the finish photo on the last-layer edge so it
# beats slicer-embedded plate-swap End G-code (SwapMod / C1M on A1 mini).
# Upstream #2547 removed this trigger; see the call site for the rationale.
_LAST_LAYER_FINISH_PHOTO = os.environ.get("BAMBUDDY_LAST_LAYER_FINISH_PHOTO", "1") != "0"
    ...
    def _update_state(self, data: dict):
        ...
        if "layer_num" in data:
            ...
            if (
                ...
                # Counts as a periodic-fallback request too (see print start).
                self._total_layers_pushall_time = time.monotonic()
                self._request_push_all()
            # Upstream #2547 removed the last-layer finish-photo trigger (it
            # fires when the final layer *starts*, and their frame-bank path
            # covers dispatcher-injected End G-code). Private fork keeps it:
            # our plate-swap End G-code is embedded by the slicer (SwapMod /
            # C1M), not injected by the dispatcher, so the frame-bank fallback
            # never applies and the FINISH-state photo would capture an
            # already-ejected plate. Toggle: BAMBUDDY_LAST_LAYER_FINISH_PHOTO=0.
            total = self.state.total_layers or 0
            if (
                _LAST_LAYER_FINISH_PHOTO
                and total > 0
                and new_layer >= total
                and old_layer < total
        ...
        if is_new_print or is_file_change:
            ...
            # Apply the pending metadata seed only when this frame brought no
            # firmware total (A1 mini: incremental pushes never carry
            # total_layer_num — see seed_total_layers()). A positive
            # total_layer_num from MQTT still overwrites this later.
            if not total_from_this_frame and self._pending_total_layers_seed > 0:
                ...
                self._pending_total_layers_seed = 0
            # Re-baseline the periodic RUNNING pushall fallback for this print.
            # When the #2702 one-shot below fires, it counts as the first
            # request — otherwise the fallback would double up in the same
            # frame; the fallback then only repeats if the answer never comes.
        ...
        # a pushall: once on the RUNNING transition, then every ~120s until a
        # positive value arrives. Runs AFTER the new-print block above so the
        # #2702 one-shot there counts as the first request and this doesn't
        # double up in the same frame. Short-interval repeats are ignored by
        # the firmware, hence the gap.
