...
def timeline_points(
    ...
    if isinstance(value, BMCUHello):
        return (TimelinePoint(
            received_at_us, link_index, "bmcu_boot", None, value.protocol_version,
        ),)
    ...
    if isinstance(value, BMCUFullStatusRecord):
        return (TimelinePoint(
            received_at_us, link_index, "full_status_record", None, value.record_type,
        ),)
    if value.record_type == 4 and value.field is not None:
        return (TimelinePoint(
            received_at_us, link_index, "state_change", value.slot, value.value or 0,
            value.severity, "bmcu",
        ),)
    return (TimelinePoint(
        received_at_us, link_index, "event", value.slot, value.record_type,
        value.severity, "bmcu",
    ),)
...
def anomaly_inputs(
    ...
    if isinstance(value, BMCUEvent) and value.severity >= 3:
        return (AnomalyInput(
            received_at_us, link_index, "reported_event", value.severity,
            value.slot, value.record_type,
        ),)
    if isinstance(value, BMCUStatus) and value.control_error:
        return (AnomalyInput(
            received_at_us, link_index, "control_error", 4, None, value.control_error,
        ),)
    return ()
