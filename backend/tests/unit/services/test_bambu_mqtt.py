...
class TestNoLastLayerFinishPhotoTrigger:
    ...
    Private fork: the last-layer trigger is deliberately KEPT (slicer-embedded
    plate-swap End G-code — see BAMBUDDY_LAST_LAYER_FINISH_PHOTO), so these
    upstream tests pin the *disabled* behavior via the toggle.
    """

    @pytest.fixture(autouse=True)
    def _disable_last_layer_trigger(self, monkeypatch):
        from backend.app.services import bambu_mqtt

        monkeypatch.setattr(bambu_mqtt, "_LAST_LAYER_FINISH_PHOTO", False)
...
class TestLastLayerFinishPhotoTriggerEnabled:
    """Private fork (#1867): with the default toggle ON, the last-layer edge
    fires the finish photo so it beats slicer-embedded plate-swap End G-code
    (SwapMod / C1M on A1 mini), and fires only once."""
    ...
    def test_last_layer_edge_fires_once(self, mqtt_client):
        events = []
        mqtt_client.on_finish_photo_moment = lambda data: events.append(data)
        ...
        assert [e["trigger"] for e in events] == ["last_layer"]
