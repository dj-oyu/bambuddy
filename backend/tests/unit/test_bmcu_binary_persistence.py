...
async def test_drop_range_advances_global_ack_idempotently(tmp_path) -> None:
    ...
    async with test_engine.begin() as connection:
        await connection.run_sync(Base.metadata.create_all, tables=[
            table for table in Base.metadata.sorted_tables if table.name.startswith("bmcu_binary_")
        ])
    ...
    hello = Hello(
        "monitor-drop", "1", (HelloLink(0, "bmcu-a"),),
        (HelloReplayRange(boot, 1, 5),), b"x" * 32,
    )
    assert await persistence.register_hello(hello, boot) == 0
    status = type(status)(
        FrameHeader(
            status.header.message_type, status.header.flags, len(status.payload), 1,
            boot, status.header.link_index,
        ),
        status.payload,
    )
    ...
    drop = type(status)(
        FrameHeader(MessageType.TRANSPORT_DROP, payload_length=len(drop_payload),
                    transport_sequence=2, pico_boot_id=boot, link_index=0xFF),
        drop_payload,
    )
