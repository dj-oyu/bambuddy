...
from backend.app.services.bmcu_binary.bmcu_decoder import ValidatedBMCUFrame, crc16_ccitt_false
...
def main() -> None:
    ...
    unsigned = Hello(
        "pico-fixture",
        "1.0.0",
        (HelloLink(0, "bmcu-a"), HelloLink(1, "bmcu-b")),
    ...
    records = {
        ...
        "link_state.bin": encode_frame(
            FrameHeader(MessageType.LINK_STATE, transport_sequence=8, pico_boot_id=boot),
            LinkStateMessage(123456, 2, 0).encode(),
        ),
        "transport_drop.bin": encode_frame(
            FrameHeader(MessageType.TRANSPORT_DROP, transport_sequence=9, pico_boot_id=boot, link_index=0xFF),
            TransportDrop(123500, 3, 6, 4, 1).encode(),
        ),
        "ack.bin": encode_frame(
            FrameHeader(MessageType.ACK, pico_boot_id=boot, link_index=0xFF), Ack(boot, 42).encode()
        ),
        "ack_reject.bin": encode_frame(
            FrameHeader(MessageType.ACK, pico_boot_id=boot, link_index=0xFF), Ack(boot, 41, (Reject(42, 1),)).encode()
        ),
        "diagnostic_unknown_tags.bin": encode_frame(
            FrameHeader(MessageType.PICO_DIAGNOSTIC, transport_sequence=10, pico_boot_id=boot, link_index=0xFF),
            encode_tlvs((TLV(240, 1, b"\x01"), TLV(241, 7, b"future"))),
        ),
        "pico_log_utf8.bin": encode_frame(
            FrameHeader(MessageType.PICO_LOG, transport_sequence=11, pico_boot_id=boot, link_index=0xFF),
            PicoLog(5, 9000, 3, "uart", "通信警告", encode_tlvs((TLV(1, 1, b"\x02"),))).encode(),
        ),
    }
    ...
    control_header = FrameHeader(
        MessageType.CONTROL,
        payload_length=len(control_base.unsigned_payload()) + 32,
        pico_boot_id=boot,
        link_index=0,
    )
    control = Control(
        control_base.command_sequence,
        control_base.issued_at_us,
        control_base.ttl_ms,
        control_base.command,
        control_base.arguments,
    ...
    records["recovered_replay.bin"] = encode_frame(
        FrameHeader(
            MessageType.PICO_LOG, flags=1 | 16, transport_sequence=2, pico_boot_id=0x8877665544332211, link_index=0xFF
        ),
